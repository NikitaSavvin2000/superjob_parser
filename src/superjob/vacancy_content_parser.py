...
def parse_job_info(driver, link):
    try:
        ...
        salary = next(
            (x for x in perhaps_salary_position
             if re.search(r'\d.*\d', x) and not re.search(r'\bгод[а]?\b', x.lower())),
            None
        )
        if salary is None:
        ...
        title = (
            perhaps_salary_position[perhaps_title_index - 1]
            if perhaps_title_index and perhaps_title_index > 0 else None
        )
        ...
        busyness = next(
            (s for s in perhaps_experience_busyness_position if busyness_pattern.search(s)),
            None
        )
        ...
        experience_busyness = [
            s for s in perhaps_experience_busyness_position if experience_pattern.match(s)
        ]
        experience_busyness_parts = (
            experience_busyness[0].split(',') if experience_busyness else []
        )
        ...
        if busyness is None:
            busyness = next(
                (s for s in experience_busyness_parts if re.search(r'занят|занятость|работа', s, re.I)),
                None
            )

        location = next(
            (s for s in perhaps_location_position if city_pattern and city_pattern.search(s)),
            None
        )
